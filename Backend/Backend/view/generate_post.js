...
module.exports = async (news, config) => {
	...
	try {
		...
		if(news.category == "") root.querySelector("#title").set_content("<strong>" + news.title + "</strong>")
		else root.querySelector("#title").set_content("<strong>" + utf8.decode('\xe3\x80\x90') + news.category + utf8.decode('\xe3\x80\x91') + news.title + "</strong>")
